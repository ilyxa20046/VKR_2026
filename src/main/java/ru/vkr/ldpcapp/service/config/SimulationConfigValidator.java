...
public class SimulationConfigValidator {
    ...
    public void validate(SimulationConfig c) {
        ...
        int k = SimulationConfigFactory.getProfileInfoWordLength(c.getLdpcProfile(), c.getLiftingSize());
        if (c.getInfoBlockLength() < k || c.getInfoBlockLength() % k != 0) {
            throw new IllegalArgumentException("Длина информационного блока должна быть кратна " + k + ".");
        }

        if (c.getSnrStep() <= 0.0) throw new IllegalArgumentException("Шаг SNR должен быть положительным.");
        if (c.getBlocks() < 20) throw new IllegalArgumentException("Количество блоков должно быть не меньше 20.");
        if (c.getMaxIterations() < 2) throw new IllegalArgumentException("Число итераций декодера должно быть не меньше 2.");
        if (c.getNormalization() < 0.5 || c.getNormalization() > 1.0) {
            throw new IllegalArgumentException("Коэффициент normalized min-sum должен находиться в диапазоне 0.50..1.00.");
        }
        if (c.getSeed() < 1) throw new IllegalArgumentException("Seed генератора должен быть положительным.");
        if (c.getMinErrorEventsPerSnr() < 1) throw new IllegalArgumentException("min error events per SNR должен быть >= 1.");
        if (c.getMaxBlocksPerSnr() < c.getBlocks()) throw new IllegalArgumentException("max blocks per SNR должен быть >= blocks.");
